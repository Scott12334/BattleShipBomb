void loadScore(String textFile)
{
  String loadFile=textFile;
  String[] inputs= loadStrings(loadFile);
  for (int i=0; i<10; i++)
  {
...
void saveScore1(String saveFile)
{
  if (player1Won==true && save==false)
  {
    if (score>scores[0])
    {
      scores[0]=score;
  ...
  {
    if (score2>scores[0] && save==false)
    {
      scores[0]=score2;
      scoreBoardNames[0]=": "+name2;
      changedSpot=0;
      for (int i=0; i<9; i++)
      {
        if (scores[i]>scores[i+1])
  ...
  saveStrings(saveFile, outputText);
  save=true;
}
